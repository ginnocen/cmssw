...
#include "HeavyIonsAnalysis/JetAnalysis/interface/HiBmesonAnalyzer.h"
...
HiBmesonAnalyzer::HiBmesonAnalyzer(const edm::ParameterSet& iConfig) :
  geo(0)
{
...
HiBmesonAnalyzer::~HiBmesonAnalyzer() { }



void
HiBmesonAnalyzer::beginRun(const edm::Run& run,
				 const edm::EventSetup & es) {}

void
HiBmesonAnalyzer::beginJob() {
...
void
HiBmesonAnalyzer::analyze(const Event& iEvent,
  ...
  LogDebug("HiBmesonAnalyzer")<<"START event: "<<event<<" in run "<<run<<endl;
...
//--------------------------------------------------------------------------------------------------
void HiBmesonAnalyzer::fillL1Bits(const edm::Event &iEvent)
{
...
//--------------------------------------------------------------------------------------------------
void HiBmesonAnalyzer::fillHLTBits(const edm::Event &iEvent)
{
...
inline void HiBmesonAnalyzer::getProduct(const std::string name, edm::Handle<TYPE> &prod,
					       const edm::Event &event) const
{
  ...
  if (!prod.isValid())
    throw edm::Exception(edm::errors::Configuration, "HiBmesonAnalyzer::GetProduct()\n")
      << "Collection with label '" << name << "' is not valid" <<  std::endl;
}

//--------------------------------------------------------------------------------------------------
template <typename TYPE>
inline bool HiBmesonAnalyzer::getProductSafe(const std::string name, edm::Handle<TYPE> &prod,
						   const edm::Event &event) const
{
...
int
HiBmesonAnalyzer::getPFJetMuon(const pat::Jet& pfJet, const reco::PFCandidateCollection *pfCandidateColl)
{
...
double
HiBmesonAnalyzer::getPtRel(const reco::PFCandidate lep, const pat::Jet& jet )
{
...
void
HiBmesonAnalyzer::saveDaughters(const reco::GenParticle &gen){
...
void
HiBmesonAnalyzer::saveDaughters(const reco::Candidate &gen){
...
double HiBmesonAnalyzer::getEt(math::XYZPoint pos, double energy){
  double et = energy*sin(pos.theta());
  return et;
}

math::XYZPoint HiBmesonAnalyzer::getPosition(const DetId &id, reco::Vertex::Point vtx){
  const GlobalPoint& pos=geo->getPosition(id);
  math::XYZPoint posV(pos.x() - vtx.x(),pos.y() - vtx.y(),pos.z() - vtx.z());
  return posV;
}

int HiBmesonAnalyzer::TaggedJet(Jet calojet, Handle<JetTagCollection > jetTags ) {
  double small = 1.e-5;
  int result = -1;
...
DEFINE_FWK_MODULE(HiBmesonAnalyzer);
